...
class Solution(object):
    def twoSum(self, nums, length, lindex, target):
        result = []
        dct = {}

        for i in range(lindex, length):
            other = target - nums[i]
            if other in dct and dct[other] != i:
                result.append([other, nums[i]])
            dct[nums[i]] = i
        return result
    ...
    def threeSum(self, nums):
        ...
        for i in range(nums_len-2):
            if i > 0 and nums[i] == nums[i-1]:
                continue
            ans = self.twoSum(nums, nums_len, i+1, -nums[i])
            for lst in ans:
                lst.append(nums[i])
                lst.sort()
    ...
    def romanToInt(self, s):
        ...
        for i in range(length):
            if (i == length - 1) or num_map[s[i]] >= num_map[s[i+1]]:
                res += num_map[s[i]]
            else:
                res -= num_map[s[i]]
        return res
    ...
    def isValid(self, s):
        ...
        for i in range(len(s)):
            if stack and stack[-1] in bracket_map and bracket_map[stack[-1]] == s[i]:
                stack.pop()
            else:
                stack.append(s[i])
        return not stack
    ...
    def countAndSay(self, n):
        ...
        for i in range(n-1):
            s = pattern.sub(lambda m: str(len(m.group(0))) + m.group(1), s)
        return s
